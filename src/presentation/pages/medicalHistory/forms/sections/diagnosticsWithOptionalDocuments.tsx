...
import { Stethoscope, Search, FileText, CheckCircle, AlertCircle, Upload, Plus } from "lucide-react";
import { MedicalHistoryDocumentUpload } from "../../../document/components/medicalHistoryDocumentUpload";
import type { SectionProps } from "../../../../../core/types/medicalHistory";

export function DiagnosticsWithOptionalDocuments({ data, onUpdate, isReadOnly = false, errors }: SectionProps) {
    ...
    return (
        <div className="space-y-6">
            {/* Sección Principal de Diagnósticos */}
            ...
            {/* Botón para mostrar/ocultar sección de documentos */}
            {!isReadOnly && (
                <div className="bg-blue-50 rounded-lg border border-blue-200 p-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-blue-700">
                                    Opcional: Adjunte resultados de laboratorio, imágenes, etc.
                                </p>
                            </div>
                        </div>
                        <button
                        ...
                        >
                            <Plus className={`w-4 h-4 transition-transform ${showDocuments ? 'rotate-45' : ''}`} />
                            {showDocuments ? 'Ocultar' : 'Agregar'} Documentos
                        </button>
                    </div>
                </div>
            )}

            {/* Sección de Documentos Colapsable */}
            {showDocuments && !isReadOnly && (
                                <MedicalHistoryDocumentUpload
                    patientId={data.patientInfo?.id}
                    onUploadSuccess={(_documentId: string) => {
                        // El hook interno manejará la actualización
                    }}
                    onUploadError={(_error: string) => {
                        // El hook interno manejará los errores
                    }}
                />
            )}
        </div>
