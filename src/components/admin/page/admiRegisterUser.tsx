...
import type { RegisterUserDto } from "../../../services/patientService" // Importar correctamente el tipo
...
const schema = yup.object().shape({
    ...
    role: yup.string().required("El rol es obligatorio"), // Cambié a obligatorio para ajustarse
})
...
export function AdminRegisterUser() {
    ...
    // Uso de useForm con tipado correcto
    ...
    const onSubmit: SubmitHandler<RegisterUserDto> = async (data: RegisterUserDto) => {
        setLoading(true)
        try {
    ...
    return (
        <SidebarProvider>
            <div className="flex min-h-screen bg-gray-50">
                ...
                {/* Sidebar */}
                <div
                    className={`w-64 bg-white shadow-md md:block ${sidebarOpen ? "block" : "hidden"}`}
                >
                    <AdminSidebar />
                </div>
